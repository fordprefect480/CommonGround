...
When `ASPNETCORE_ENVIRONMENT=Development`, the API on startup will:

1. Apply any pending EF Core migrations (`Database.MigrateAsync`).
2. Seed an admin user if one does not exist:
   - **Email:** `admin@local`
   - **Password:** `Password123!`
   - **Role:** `Admin`

Sign in at the SPA's `/login` page using those credentials to access `/admin`.

> **Production** runs do **not** auto-migrate or seed. Run migrations explicitly (see [Database migrations](#database-migrations)).
...
| `ASPNETCORE_ENVIRONMENT` | env var                                | `Development` enables auto-migrate, dev seed, OpenAPI UI | `Development` (in launchSettings) |
...
# Apply pending migrations to a target database (Development auto-applies on startup)
dotnet ef database update --project CommonGround.Server
```

Migrations are run automatically at startup in Development. Production deploys should run them as a deploy step.
...
| `/api/config`       | Public app config (garden name)                      |
